...
## Database (Supabase)
**providers** — id (uuid, = auth.users id), business_name, contact_name, email, phone, city, care_type, ages_served (text), price_range (text, free-form display string), starting_price (numeric, nullable — plain number used only for sorting, entered alongside price_range), bio, photo_url, is_approved (boolean, default false), review_status (text: 'pending'|'approved'|'rejected', default 'pending', kept in sync with is_approved by admin.html), created_at.
